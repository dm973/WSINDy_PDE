...
function [W,G,b,M,lambda_learned,tags_pde_G,lib_list_G,true_nz_weights,resid,dW,its_all,thrs_EL,ET_wsindy,tags_pde,lib_list,pdx_list,lhs_ind, Cfs_x,Cfs_t,dx,dt,p_x,p_t,sub_inds,scales,M_full,Theta_pdx] = wsindy_pde_fun(U_obs,xs,lambda,gamma,true_nz_weight_tags,...
    lhs,max_dx,max_dt,polys,trigs,custom_add,custom_remove,use_all_pt,use_cross_dx,...
    toggle_scale,supp_phi_x,supp_phi_t,s_x,s_t,tols,phi_class)
...
if length(lambda)>1
    [W,G,b,resid,dW,its_all,thrs_EL,M] = wsindy_pde_RGLS_seq(lambda,gamma,Theta_pdx,lhs_ind,true_nz_weights,M_full);
else
    [W,G,b,resid,dW,its_all,thrs_EL,M] = wsindy_pde_RGLS(lambda,gamma,Theta_pdx,lhs_ind,true_nz_weights,M_full);
end
ET_wsindy = toc;
